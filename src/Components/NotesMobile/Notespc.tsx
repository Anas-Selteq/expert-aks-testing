...
function NotesPc() {
    ...
    const fetchDataGetTypesAPI = async () => {
      try {
        const response = await axios.get(
          `https://gateway.findanexpert.net/notesapi_svc/pv/types/`
        );
  ...
  const firstnotesrequesttosend = async () => {
    ...
    try {
      const response = await axios.post(
        "https://gateway.findanexpert.net/notesapi_svc/pv/notes/",
        formData,
        {
          headers: {
  ...
  const DeleteNotesbyid = async (item: any) => {
    ...
    try {
      const response = await axios.delete(
        `https://gateway.findanexpert.net/notesapi_svc/pv/notes/?id=${item}&created_by=${profile?.externalCustomerId}&created_by_type=2`,
        {
          headers: {
            "Content-Type": "multipart/form-data", // Ensure you use multipart/form-data for form data
          },
  ...
  useEffect(() => {
    ...
    const fetchDataFromAPI = async () => {
      try {
        const response = await axios.get(
          `https://gateway.findanexpert.net/notesapi_svc/pv/notes/?id=${getpurchaseorderid}&type=purchase_order_id&timezone=Asia/Karachi`
        );
              ...
              return (
                <>
                  <div className="col-md-12 text-end ">
                    <p className="m-0 p-0 font_size_m">
                  ...
                  {item?.notesContent?.text_data ? (
                    <div className="col-md-12 ">
                      <div className="flex_for_notes ">
                        <p className="  mt-2  font_chat_text">
                          <span className="background-chat_tags px-2 py-2 ">
                            {item?.notesContent?.text_data}
                          </span>
                        </p>
                        <div className="ps-2">
                          <Dropdown drop="start">
                            ...
                            <Dropdown.Menu>
                              <Dropdown.Item
                                onClick={() => {
                                  setUpdateNotesId(item?.notesContent?.note_id),
                                    setNewNotes(true);
                                }}
                              >
                                Update
                              ...
                              <Dropdown.Item
                                onClick={() =>
                                  DeleteNotesbyid(item?.notesContent?.note_id)
                                }
                              >
                                Delete
                              </Dropdown.Item>
                    ...
                    <div className="col-md-12 px-2 py-2">
                      <div className="flex_for_notes">
                        <img
                          className="img-fluid background-chat_tags px-2 py-2"
              ...
              selectedFileImage === null ? (
                <>
                  <input
                    ...
                    value={inputtextval}
                    onChange={(event: any) =>
                      setinputtextval(event?.target.value)
                    }
                  />
                  <div className="input-group-append">
                    <button
                      className="btn btn-light px-2 pt-2 background_buttons_input"
                      type="button"
                      onClick={handleShow}
                    >
                    ...
                    <button
                      className="btn btn-light pe-0 ps-2  pt-2 background_buttons_input"
                      type="button"
                ...
                <span className="px-2 py-1">
                  {selectedFile?.name} {selectedFileAudio?.name}{" "}
                </span>
              )}
            </div>
          </div>
          <div className="col-md-1 col-2 pe-2 m-auto">
            {loading === false ? (
              <>
                {NewNotes === false ? (
                  <img
                    onClick={firstnotesrequesttosend}
                    className="img-fluid img_border_rad"
                    src="/imagess/navi.png"
                  />
                ) : (
                  <>
                    {/* <span onClick={updatenotesrequesttosend}>HIt</span> */}
                    <img
                      onClick={imguploadrequesttosend}
                      className="img-fluid img_border_rad"
                      src="/imagess/navi.png"
                    />
                  </>
              ...
              <div className="col-md-12 col-12">
                <div className="col-md-12  text-center py-3 border_for_modal">
                  <label
                    ...
                    onClick={handleCustomInputClickImage}
                  >
