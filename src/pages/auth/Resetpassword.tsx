...
import { resetOrAddUserPassword, resettPassword } from "@/helper";
...
function Resetpassword() {
  ...
  const handleClick = (e: any) => {
    ...
    if (confirmPassword !== passwordValue) {
      ...
      setLoader(true);
      resettPassword(query.userId, false, passwordValue, userOTP)
        .then((result) => {
          if (result?.code === 0) {
        ...
        <>
          ...
          <form onSubmit={handleClick}>
            ...
            {loader ? (
              ...
              <div >
                {passwordValue === confirmPassword && /^(?=.*\d)(?=.*[!\"£$@#%^&*])(?=.*[a-zA-Z]).{8,}$/.test(passwordValue) ?
                  <Button type="submit" width="340px"  style={passwordValue === confirmPassword && /^(?=.*\d)(?=.*[!\"£$@#%^&*])(?=.*[a-zA-Z]).{8,}$/.test(passwordValue) ? { backgroundColor: "#DF1919" } : { backgroundColor: "#ef9393 ", cursor: "no-drop" }}>
                    Save & Continue
                  </Button> :
                  <Button  width="340px" onClick={passwordcheck}  style={passwordValue === confirmPassword && /^(?=.*\d)(?=.*[!\"£$@#%^&*])(?=.*[a-zA-Z]).{8,}$/.test(passwordValue) ? { backgroundColor: "#DF1919" } : { backgroundColor: "#ef9393 ", cursor: "no-drop" }}>
                    Save & Continue
                  </Button>
                }
              </div>
