...
import { getIndustryServiceByIndustryId, getPurchaseOrderWithId, postPurchaseOrder } from "@/helper";
import {
...
const ServicesByIndustryId = () => {
  ...
  const handleFlowScreen = (flow: any, serviceeattribute: any) => {
    console.log('service12', serviceeattribute)
  ...
  useEffect(() => {
    // Initialize counters array with 0 for each service
    setCounters(new Array(listOfServices.length).fill(1));
  }, [listOfServices]);
  ...
  const increment = (index: number) => {
    const updatedCounters: any = [...counters];
    updatedCounters[index] += 1;
    setCounters(updatedCounters);
    console.log(`Incremented count for service at index ${index}: ${updatedCounters[index]}`);
    localStorage.setItem("main_quantity", updatedCounters[index]);
  };
    ...
    <Layout2>
      ...
      <div className="col-md-12 custom_padding_services_pages">
        <div className="col-md-12">
          <p className="mb-1 p-0 service_choose">Please choose any Service</p>
        </div>
        <div className="row">
